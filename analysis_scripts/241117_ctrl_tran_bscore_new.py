...
# %%
bscore_df = pd.read_csv(Path(EXP_DIR).joinpath('ctrl_trans_var_240927', 'brainscore_results.csv'), index_col=0)
...
for i_s, state in enumerate(product(dset_list, task_set_list, seed_list)):
    ...
    model_dict['model'].append(f'ctrl_trans_var_240927-resnet18-{i_s}-batch--1')
...
# %%
plot_task_names = ['Distance', 'Rotation', 'Dis. Rot.', 'Category']
plot_data = {
...
fig.savefig(Path(FIG_DIR).joinpath('ctrl_tran_var_model_brainscore.pdf'), transparent=True, bbox_inches='tight')
